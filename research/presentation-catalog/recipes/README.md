...
└── <source>/                    # Generated by setup task
    ...
    │   ├── generate-chunks.md
    │   └── consolidate.md
    └── <chunk>/                 # Generated by generate-chunks task
...
- **`_generators/`** — Templates that guide task behavior (single responsibility each)
- **`_context.md`** — Metadata and guidance (hierarchical, at each level)
- **`_prompt/`** — Staged prompts; each level stages prompts for next level

### Two-Phase Model

**Generation (top-down):** Run prompts to create structure and stage next-level prompts.
**Execution (bottom-up):** Run extraction prompts at leaves, then consolidate.

See the top-level `presentation-catalog/README.md` for the full model.

## Setting Up a New Source

Sources must be prepared before extraction. See `methodology/source-preparation.md`.

Once a source is prepared (validated `_source.yaml` in `reference/<source>/`), set it up
for recipe extraction using a fresh agent with this prompt:

```
Follow research/presentation-catalog/recipes/_generators/setup-source.md
with the source at
```

The setup task reads metadata from `_source.yaml`, examines the source material, and
creates `_context.md` with extraction guidance. No seed prompts or templating needed —
the generator contains all static references, and metadata comes from the source.
