...
| # | Status | Applied | Item |
|---|--------|---------|------|
| 1 | decided | no | Implementation language: Rust |
| 2 | decided | no | Canonical binary serialization (format TBD); replaces YAML. Exception: annotations use line-oriented text (merge strategy requires it) |
| 3 | tentative | no | State dir: `.git/apparatus/`, store: `.git/apparatus/store/` (removes devenv requirement) |
| 4 | decided | no | Git notes for annotations: op-log in `refs/notes/apparatus`, `cat_sort_uniq` merge |
| 5 | pending | no | Index staleness: stored ref SHA instead of generation counter |
| 6 | pending | no | Structured commit trailers for audit trail |
| 7 | pending | no | Containment model assumed composition without experimental validation |
| 8 | **breaking** | no | Concurrent mutations required (multiple local/remote sources) |
| 9 | **breaking** | no | Scale: ~300k objects within months, not "hundreds to low thousands" |
| 10 | pending | no | Distributed divergence: detection, classification, and resolution strategy |
| 11 | interim | no | Claim mechanism + remote atomic updates for critical state changes during dev |
| 12 | decided | no | Annotation type removed from Section 4 object list; lives entirely in notes, not containment hierarchy |
| 13 | confirmed | no | Commit chain is the op log for all non-annotation structures; no separate op-log needed |
| 14 | confirmed | no | `created-at` is display/informational only; correctness ordering from commit chains |
| 15 | pending | no | Owner semantics on structures (explicit owner field; affects data model, not just coordination) |
| 16 | confirmed | no | Citation model `(assigned-id, content-SHA)` validated; stronger justification via divergence traceability |
